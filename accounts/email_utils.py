...
def welcome_email(username: str, recipient_name: str = "") -> tuple:
    """Build a welcome email for newly created staff accounts."""
    greeting = f"Hi {recipient_name}," if recipient_name else "Hi there,"

    plain = (
        f"{greeting}\n\n"
        f"A staff account has been created for you on {BRAND_NAME}.\n\n"
        f"Username: {username}\n"
    ...
    html = _base_html(f"""
        ...
        <tr>
          <td style="padding:12px 20px;">
            <table cellpadding="0" cellspacing="0" width="100%">
              ...
              <tr>
                <td style="font-size:16px;font-weight:700;color:#1a1a2e;">{username}</td>
              </tr>
            </table>
          </td>
        </tr>
      ...
      <p style="font-size:12px;color:#dc2626;margin:0 0 16px;font-weight:500;">
        &#9888; You will need to set your password on first login.
      </p>
      <hr style="border:none;border-top:1px solid #e8ecf1;margin:20px 0;">
      <p style="font-size:12px;color:#9ca3af;margin:0;line-height:1.5;">
        Please visit the system to log in. If you did not expect this, contact the clinic.
